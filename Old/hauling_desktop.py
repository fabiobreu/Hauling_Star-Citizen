...
def load_config():
    ...
    # Load Patterns
    lang = config.get("log_language", "en")
    pattern_file = os.path.join(BASE_DIR, f"patterns_{lang}.json")
    if os.path.exists(pattern_file):
        try:
            with open(pattern_file, 'r', encoding='utf-8') as pf:
                PATTERNS.update(json.load(pf))
                print(f"Loaded patterns: {pattern_file}")
...
class HaulingMonitor:
    ...
    def process_line(self, line):
                ...
                if text_match:
                    ...
                    if PATTERNS["contract_accepted"] in notification_text:
                        ...
                        if obj_match:
                            ...
                            type_str = "PICKUP" if action in ['COLLECT', 'PICKUP', 'COLETAR', 'PEGAR', 'ENTREGAR', 'DEIXAR', 'TRANSPORTAR'] else "DELIVERY"
        ...
        # 4. Reward (Standalone)
        if "aUEC" in line:
             reward_match = re.search(PATTERNS.get("reward_regex", r"Awarded\s+(\d+)\s+aUEC"), line)
             if reward_match:
                 amount = int(reward_match.group(1))
                 data_store["total_earnings"] += amount
                 # Add to last finished mission if possible? For now just track total.
                 save_state()
